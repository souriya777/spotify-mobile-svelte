<script>
  ...
  import { displayFilterMyLib } from '@js/store';
...
<div>
  ...
  {#if $displayFilterMyLib.trackOn}
    <h3>1st tracks</h3>
    <CollectionTrack items={firstTracks} />
  {/if}

  {#if $displayFilterMyLib.artistOn}
    <h3>1st artists</h3>
    <ListArtist items={firstArtists} />
  {/if}

  {#if $displayFilterMyLib.playlistOn}
    <h3>1st playlists</h3>
  {/if}

  {#if $displayFilterMyLib.albumOn}
    <h3>1st albums</h3>
