<script>
  ...
  $: searchUrl = $searchQuery ? `/search/${$searchQuery}` : '/search';
</script>

<nav>
  <ul>
    <li>
      <NavLink to="/" isActive={/^\/$/gi.test($currentPath)}>
    ...
    <li>
      <NavLink to={searchUrl} isActive={/search/gi.test($currentPath)}>
        <svg height="24" width="24" viewBox="0 0 24 24">
          <path
    ...
    <li>
      <NavLink to="/my-lib" isActive={/my-lib/gi.test($currentPath)}>
        <svg height="24" width="24" viewBox="0 0 24 24">
          <path
